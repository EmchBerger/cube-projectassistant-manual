...
\begin{compactitem}
	...
	\item Die Benachrichtigungs-Funktion wurde durch weitere Optionen ergänzt
\end{compactitem}

\vspace{\baselineskip}

\textbf{Adressliste/CRM:}
\begin{compactitem}
	\item Inline-Bearbeitung: Die Einträge können in der Übersicht bearbeitet werden
	\item Neue Filterfunktionen (Suche nach E-Mailadressen)
	\item Teilnehmerlisten für CRM direkt aus Adressliste erstellen
	\item Teilnehmerlisten können gelöscht werden
\end{compactitem}
...
\begin{compactitem}
  \item Mobile-Handling für Pendenzen: Ansicht, Bearbeiten, Filtern, Dokumente verknüpfen und hochladen, Listen/PDF generieren
...
\begin{compactitem}
	\item Dokumente per CUBE ProjectAssistant oder per Outlook (Vorlage) versenden
  \item Dokumente wahlweise als Attachement, als Download-Link oder Link zum Dokumenteneintrag versenden
	\item Repro-Versand: Dokumente / Pläne wahlweise als Link für CUBE-Benutzer, als Link ohne Login (öffentlicher Link mit 30 tägiger Gültigkeit) oder als Anhang versenden
	\item Es können mehrere, parallele Workflows (Status) für das Dokumentenmanagement verwendet werden
\end{compactitem}
...
\begin{compactitem}
  \item GUI-Modul für die Konfiguration der Workflows (Statussysteme)
	\item Unterstützung mehrerer, paralleler Workflows bei der Dokumentenablage
