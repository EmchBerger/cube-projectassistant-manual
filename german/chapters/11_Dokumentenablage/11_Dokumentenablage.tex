...
In der Übersicht kann wie gewohnt nach Einträgen gesucht oder bei einigen Spalten können Filter gesetzt und das Suchresultat nach Spalten sortiert werden. Weiter ist in der Übersicht ersichtlich wie viele Dokumente in einem Planungsdossier enthalten sind und ob das Dossier gesperrt ist, resp. durch wen. Wenn Sie hier auf das Plussymbol \includegraphics[height=12pt]{/Icons/Plussymbol.jpg} klicken, um ein neues Planungsdossier zu erstellen, wird das Dossierformular geöffnet (siehe auch Kapitel \ref{bkm:Ref442544219}).\\
...
\textbf{Beispiel:} In der Tagstruktur (Dokumentenablage) ist der Tag Typ 'Dokumentenart' mit seinen Tags ersichtlich. Bei einem Planungsdossier wird lediglich der Dokumenttyp angehängt:
...
Im Bearbeitungsmodus eines Dossier (oder beim Erstellen eines Planungsdossier) wird die Option 'ist ein Planungsdossier' angewählt \includegraphics[height=12pt]{/Icons/checkbox_markiert.jpg} \col{(1)}. Zudem werden die benötigten Tag Typen hinzugefügt \col{(2)}. Geben Sie in die Zeile 'Zugehörige Tag Typen' die Ihnen bekannten Suchbegriffe ein und wählen Sie aus den Vorschlägen aus.

Mit 'Übernehmen' werden die Änderungen gespeichert und mit dem Listensymbol \includegraphics[height=12pt]{/Icons/Listensymbol_zurueck.jpg} kehren Sie zur Übersicht zurück.

\vspace{2cm}

\textbf{Die Anwendung von Planungsdossiers}
...
Wurde ein Dokument einem Planungsdossier hinzugefügt, können Sie mit Klick auf den Pfeil (\includegraphics[height=11pt]{/Icons/Pfeil_l.jpg}) \col{(2)} direkt in dieses Planungsdossier wechseln. Das oder die Planungsdossiers werden aufgelistet und mit Klick auf den blauen Textlink \col{(3)} wird direkt das entsprechende Planungsdossier geöffnet.
...
Detailiertere Informationen zu Planungsdossier finden Sie in Kapitel \ref{bkm:Ref2018073102}.
...
Wurden alle nötigen Eingaben gemacht, werden das neue Dokument und die Änderungen der Einträge mit dem \includegraphics[height=12pt]{/Icons/B_Uebernehmen.jpg}-Button (unten links oder oben in der Mitte des Browserfenster) gespeichert. Der alte Stand (Dokument und Metadaten) bleiben gespeichert und können in der 'Änderungshistorie' der Dokumentenansicht \includegraphics[height=12pt]{/Icons/Lupe.jpg} jederzeit wieder aufgerufen werden.
...
Klicken Sie auf das Versandsymbol \includegraphics[height=12pt]{/Icons/dk_senden.jpg}, um sämtliche Dokumente im Dokumentenkorb an eine oder mehrere Emailadressen zu versenden. Sie können vor dem Absenden immer noch Dokumente löschen, welche Sie nicht versenden wollen.
...
\begin{figure}[H]
\center{\includegraphics[width=.4\linewidth]{../chapters/11_Dokumentenablage/pictures/dok_DruckauftrInfo.jpg}}
% \caption{Neues Dokument: Zugriffsrechte}
% \label{fig:speciation}
\end{figure}

Klicken Sie auf das \euro{-Symbol} \col{(1)} erhalten Sie eine Excelliste, welche die gefilterten Druckaufträge enthält:
...
Sie können hier Anpassungen vornehmen und im Arbeitsblatt 'Zusammenfassung' eine Übersicht gewinnen, welche beispielsweise für eine Verrechnung verwendet wird.

\begin{figure}[H]
\center{\includegraphics[width=.45\linewidth]{../chapters/11_Dokumentenablage/pictures/dok_DruckauftrExcelZu.jpg}}
...
\begin{figure}[H]
\center{\includegraphics[width=1\linewidth]{../chapters/11_Dokumentenablage/pictures/dok_DruckauftrDetails.jpg}}
\caption{Detailinformationen zum Druckauftrag / Lieferschein herunterladen}
% \label{fig:speciation}
\end{figure}

Mit Klick auf 'Lieferschein' \col{(1)} wird der Lieferschein eingeblendet, mit erneutem Klick \col{(4)} , wieder ausgeblendet. Die Beilagen des Druckauftrags werden aufgelistet und mit Klick auf den Link \col{(2)} können Sie das Dokument öffnen oder speichern. Mit dem Lupensymbol \includegraphics[height=12pt]{/Icons/Lupe.jpg} \col{(3)} wird das Dokument in der Dokumentenablage im Betrachtungsmodus geöffnet. Klicken Sie auf den Link des Lieferscheins \col{(5)}, um diesen Herunterzuladen. Mit dem \includegraphics[height=12pt]{/Icons/X_Button.jpg}-Button \col{(6)} verlassen Sie das Fenster wieder und kehren zur Druckauftragsübersicht.
